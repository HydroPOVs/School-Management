...
class Student:
    ...
    def subjectopt(self, isback=False):
        ...
        if count == 0:
            ...
            for row in data:
                rollno, n = row
            st2 = "SELECT Subject_1, Subject_2, Subject_3,Subject_4, Subject_5 from Student_subjectopt where Roll_no={}".format(
                no)
            cursor.execute(st2)
    ...
    def fulldetails(self, isback=False):
        ...
        if count == 0:
            ...
            for row in data:
                rollno, n, f, m, passwd = row
            st2 = "SELECT Age, Address, Contact_No, Class_10th_Marks, Class_12th_roll_no, Fathers_Occupation from Student_data where Roll_no={} ".format(
                no)
            cursor.execute(st2)
            data2 = cursor.fetchall()
            for row in data2:
    ...
    def result(self, isback=False):
        ...
        if count == 0:
            ...
            for row in data:
                rollno, n = row
            st2 = "SELECT Marks_in_subject_1, Marks_in_subject_2, Marks_in_subject_3, Marks_in_subject_4, Marks_in_subject_5,Percentage, Overall from Student_marks where Roll_no={}".format(
                no)
            cursor.execute(st2)
            data2 = cursor.fetchall()
